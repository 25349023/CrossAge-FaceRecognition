...
class TestCALFWDataset:
    ...
    def output_similarity(self, filelist_path, out_path):
        with open(os.path.join(self.root, filelist_path), 'r', encoding='utf-8', newline='') as f, \
                open(out_path, 'w', encoding='utf-8', newline='') as f2:
    ...
    def gen_all_features_from(self, filelist_path):
        ...
        with torch.no_grad(), open(os.path.join(self.root, filelist_path), 'r', encoding='utf-8', newline='') as f:
            for file, in csv.reader(f):
                image = Image.open(os.path.join(self.root, 'ForGrouping', file))
                if self.transform is not None:
                    image = self.transform(image)
...
if __name__ == '__main__':
    ...
    model = model_insightface.GroupMobileFaceNet(512).eval()
    model.load_state_dict(torch.load('./ckpt/'+args.ckpt))
    TestCALFWDataset(
        '../data/calfw/', 'ForTesting/Test_PairList.csv',
        f'Test_team08_results_{args.ckpt.split(".")[0].split("-")[-1]}.csv', model,
        transform=T.Compose([T.Resize(112), T.ToTensor(),
                             T.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])])
    )
    GroupingCALFWDataset(
        '../data/calfw/', 'ForTesting/Test_BonusList.csv',
        f'Test_team08_bonus_results_{args.ckpt.split(".")[0].split("-")[-1]}.csv', model,
        transform=T.Compose([T.Resize(112), T.ToTensor(),
                             T.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])])
    )